testing = [{:name=>"Stephen Spielberg",
  :movies=>
   [{:title=>"Jaws",
     :studio=>"Universal",
  ...
  :movies=>
   [{:title=>"The Godfather",
    ...
    {:title=>"Jack",
     ...
     def look_up(thing)
       #binding.pry
       pp thing[0][:movies][0][:worldwide_gross]
     end
    ...
    while list_index < testing.length do
      puts "\n\nName of hash #{list_index} belongs to #{testing[list_index][:name]}. \n\n"
      ...
      testing.length.times do
        puts " They made: "
          
          movie_index = 0
            
          while movie_index < testing[list_index][:movies].length do
            puts "\t\t #{movie_index + 1} #{testing[list_index][:movies][movie_index][:title]},"
